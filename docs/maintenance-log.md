...
### 2026-07-04 Epic TOTP 自定义码格键盘输入

- 现象：
  - 修复等待逻辑后，GitHub Actions run `28710774028` 仍在认证阶段失败。
  - 新截图显示页面已经进入 `Enter Security Code`，并展示 6 个验证码格子，但日志仍报找不到普通验证码输入框。
- 根因判断：
  - Epic 的 6 位验证码格子在当前页面实现中不一定暴露为普通 `input`，原选择器和 `fill()` 路径无法识别这些自定义码格。
  - 页面本身已经聚焦首个码格，适合在确认 MFA 页面后走键盘输入后备路径。
- 改动文件：
  - `app/services/epic_totp_service.py`
  - `docs/maintenance-log.md`
- 处理结果：
